...
{
    charge_fee(AccountSettings.fee_flat_cents, ts, " monthly fee.");
}
...
void Portfolio::apply_from_ledger(const string* tx_account_id,const int* tx_type,const long long* tx_amount_cents, int tx_count) 
{
    ...
    {
        vect.push_back({tx_type[i], tx_amount_cents[i], 0, "", tx_account_id[i]});
    }
    apply_all(vect);
}
...
long long Portfolio::total_exposure() const
{
    ...
    {
        total += i.secound->balance_cents();
    }
    return total;
}

/*
    std::vector<TxRecord> v;
    v.reserve(tx_count);
