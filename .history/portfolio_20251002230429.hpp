...
enum class TxKind {
    ...
    TransferIn ,  ///< Adding fund to an account
    TransferOut , ///< Removing fund from an account
};
...
/**
 * @struct TxRecord
 * @brief Represents a single transaction record  
 */
struct TxRecord {
    TxKind kind;
 ...
 * @brief Stores detailed information about a fund transfer transaction  
 */
struct TransferRecord {
    string from_id;
    string to_id;
 ...
 * @brief Represents a checking account that supports monthly fees and specific settings
 */
class CheckingAccount : public BaseAccount {
 public:
 /**
  * @brief Constructor for a new checking account object
  * 
  ...
  */
     ...
     * 
     * @return AccountType The typeof the account
     */
    AccountType type() const override;

    /**
     * @brief Apply the monthly fee to the account
     * 
     * @param ts Timestamp of the transaction
     */
    void apply_monthly_fee(uint64_t ts);
};

/**
 * @class SavingsAccount
 * @brief Represents a savings account that accrues interest
 */
class SavingsAccount : public BaseAccount {
 public:
 /**
  * @brief Constructor for a new savings account object
  * 
  * @param id               Unique identifier for the account
  * @param settings         Account configuration settings
  * @param opening_balance  Initial balance in cents (defaults to 0)
...
/**
 * @class Portfolio
 * @brief Manages a collection of financial accounts and provides operations
 *        for transactions, transfers, and exposure calculations
 */
class Portfolio {
    ///< Container holding all accounts, mapped by their unique ID 
 ...
 public:
    ...
    /**
     * @brief Retuen an account by its id
     * 
     * @param id  Unique identifier for the account
     * @return    A pointer to the IAccount object if found
     */
    IAccount* get_account(const string& id);

    /**
     * @brief Applies group of transactions to their corresponding accounts
     * 
     * @param txs Vector containing transaction records
     */
    void apply_all(const vector<TxRecord>& txs);

    /**
     * @brief  Applies transactions from raw ledger array
     * 
     * @param tx_account_id    Pointer to IDs array
     * @param tx_type          Pointer to types array
     * @param tx_amount_cents  Pointer to amounts cents array
     * @param tx_count         Number of transactions to apply
     */
     ...
     * 
     * @param tr Transfer record for transfer details
     * @return   true if the transfer succeeded 
     */
    bool transfer(const TransferRecord& tr);

    /**
     * @brief Calculates the total exposure across all accounts.
     *
     * This function sums up the balances or exposure amounts of all
     * managed accounts.
     *
     * @return Total exposure as a 64-bit integer.
     */
