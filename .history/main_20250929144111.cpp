#include "portfolio.hpp"
#include <iostream>

using namespace std;

int main() {
    Portfolio pf;

    /********** Create portfolio and accounts **********/
    AccountSettings chk_acc{AccountType::Checking, 0.0, 150};
    AccountSettings sav_acc{AccountType::Savings, 0.05, 0};

    pf.add_account("CHK-001", chk_acc, 0);
    pf.add_account("SAV-010", sav_acc, 500000);

    /********** Apply transactions (vector path) **********/
    vector<TxRecord> txs;
    
    txs.push_back({TxKind::Deposit, 100000, 20250101, "Deposit", "CHK-001"});
    txs.push_back({TxKind::Withdrawal, 25000, 20250102, "Withdrawal", "CHK-001"});
    txs.push_back({TxKind::Fee, 1500, 20250103, "Withdrawal", "CHK-001"});
    pf.apply_all(txs);

    cout <<endl << "CHK-001 balance: " << pf.get_account("CHK-001")->balance_cents() <<endl;

    /**********  Savings interest **********/ 
    pf.get_account("SAV-010")->post_simple_interest(31, 365, 20250131, "monthly interest");
    cout <<endl << "SAV-010 balance: " << pf.get_account("SAV-010")->balance_cents() << endl<<endl;


    /**********  Transfer between accounts **********/ 
    TransferRecord tr{"SAV-010 ", " CHK-001", 30000, 20250201, "transfer"};

    pf.transfer(tr);

    cout << "CHK-001 after transfer: " << pf.get_account("CHK-001")->balance_cents() << endl;
    cout << "SAV-010 after transfer: " << pf.get_account("SAV-010")->balance_cents() << endl;

    /**********   Apply transactions (Ledger path) **********/ 
    int size = 2;
    string id[size] = {"CHK-001", "SAV-010"};
    int types[size] = {0, 3}; // 0 -->Deposit, 3 -->Interest
    long long amounts[size] = {5000, 1000};

    pf.apply_from_ledger(id, types, amounts, size);

    /**********   Total exposure **********/ 
    cout <<endl << "Total exposure: " << pf.total_exposure() << endl<< endl;

    return 0;
}
