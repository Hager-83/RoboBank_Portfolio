...
BaseAccount::BaseAccount(const string &id, const AccountSettings &settings,
...
int64_t BaseAccount::balance_cents() const { return balance_cents_; }

const vector<TxRecord> &BaseAccount::audit() const { return audit_; }
...
void BaseAccount::deposit(int64_t amount_cents, int64_t ts, string note) {
  balance_cents_ = apply_deposit(balance_cents_, amount_cents);
  record({TxKind::Deposit, amount_cents, ts, note, id_});
}

void BaseAccount::withdrawal(int64_t amount_cents, int64_t ts, string note) {
  balance_cents_ = apply_withdrawal(balance_cents_, amount_cents);
  record({TxKind::Withdrawal, amount_cents, ts, note, id_});
}

void BaseAccount::charge_fee(int64_t fee_cents, int64_t ts, string note) {
  balance_cents_ = apply_fee(balance_cents_, fee_cents);
  record({TxKind::Fee, fee_cents, ts, note, id_});
}

void BaseAccount::post_simple_interest(int days, int basis, int64_t ts,
                                       string note) {
  long long interest = simple_interest(balance_cents_, settings_.apr, days,
                                       basis, 'E'); // Defult(Half Even)
...
/**********  CheckingAccount **********/
CheckingAccount::CheckingAccount(const string &id,
...
void CheckingAccount::apply_monthly_fee(int64_t ts) {
  charge_fee(settings_.fee_flat_cents, ts, " monthly fee.");
}

/**********  SavingsAccount **********/
SavingsAccount::SavingsAccount(const string &id,
                               const AccountSettings &settings,
                               int64_t opening_balance)
    : BaseAccount(id, settings, opening_balance) {}
...
void SavingsAccount::post_interest(int days, int basis, int64_t ts) {
  post_simple_interest(days, basis, ts, "posting interest monthly.");
}

/**********  Portfolio **********/
bool Portfolio::add_account(const string &id, const AccountSettings &settings,
                            int64_t opening_balance) {
  if (!accounts_.count(id)) {
    if (settings.type == AccountType::Checking) {
      accounts_[id] =
          make_unique<CheckingAccount>(id, settings, opening_balance);
    } else {
      accounts_[id] =
          make_unique<SavingsAccount>(id, settings, opening_balance);
    }
    return true;
  } else {
    return false;
...
bool Portfolio::transfer(const TransferRecord &tr) {
  ...
  if (!(from || to)) {
    ...
    from->apply({TxKind::TransferOut, tr.amount_cents, tr.timestamp, tr.note,
                 tr.from_id});
    to->apply(
        {TxKind::TransferIn, tr.amount_cents, tr.timestamp, tr.note, tr.to_id});
    return true;
  }
}

int64_t Portfolio::total_exposure() const {
  int64_t total = 0;
